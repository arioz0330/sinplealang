...
impl Op {
    pub fn new(s: &str) -> Result<(&str, Self), String> {
        utils::tag("+", s, "Expected mathematical operator")
            .map(|s| (s, Self::Add))
            .or_else(|_| {
                utils::tag("-", s, "Expected mathematical operator").map(|s| (s, Self::Sub))
            })
            .or_else(|_| {
                utils::tag("*", s, "Expected mathematical operator").map(|s| (s, Self::Mult))
            })
            .or_else(|_| {
                utils::tag("/", s, "Expected mathematical operator").map(|s| (s, Self::Div))
            })
    }
}
